...
class Config:
    ...
    def load(cls, path: str | None = None) -> "Config":
        ...
        if not os.path.exists(path):
            logger.warning(f"{path} 없음 → 내장 기본값 사용")
            return cls()
    ...
    def validate(self) -> None:
        ...
        if self.warmup_bars < self.trend_score.min_len:
            logger.warning(f"warmup_bars({self.warmup_bars}) < TrendScore.min_len"
                           f"({self.trend_score.min_len}) — 시작 구간 지표가 예열되지 않을 수 있습니다.")
        w = self.trend_score
        wsum = w.ewmac_weight + w.tsmom_weight + w.rsi_weight
        if abs(wsum - 1.0) > 0.01:
            logger.warning(f"TrendScore 가중 합이 {wsum:.2f}(권장 1.0) — 그대로 진행합니다.")
