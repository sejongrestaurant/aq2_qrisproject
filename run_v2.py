...
# ── 실험 조합 (여기만 편집) ─────────────────────────────────────
# Tier 2-a 부분 진입: 60점 이진 진입 → 점수 비례 경사. 전환 구간(5슬롯대, **V1 원설계**
# 평균 −1.08%)이 유일한 마이너스 상태라는 진단을 정면으로 겨냥한다(동결 V2 재산출에선 +2.83%
# 로 반전 — run_segments.py). 채택하려면 한 점이 아니라 주변값 그리드에서 plateau 여야 한다
# (규율 3) — 그래서 하한충전율·경사폭을 함께 흔든다.
