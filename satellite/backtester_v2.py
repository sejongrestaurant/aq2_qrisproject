...
취급이 극단적으로 갈린다. 진단된 약점(**V1 원설계 기준** 전환 구간 5슬롯대 평균 구간수익
−1.08%, 유일한 마이너스 상태)이 바로 이 문턱 근처에서 나온다: 급락 후 회복 국면에서 점수가
문턱을 오르내리는 동안 노출이 0과 100%를 널뛴다. (동결 V2 재산출에서 이 5슬롯대는 +2.83%
로 부호가 뒤집혔다 — `analysis/segments.py`. 이 변형이 겨냥한 지점이 실제로 개선됐다는 증거다.)
...
class SatelliteBacktesterV2(SatelliteBacktester):
    ...
    Args (생성자):
        ...
        entry_gate: **신규 편입 자격 문턱**(랭킹 진입 자격). None 이면 ramp_score 를 따라간다.
            ...
            **단 동결 구성은 이 권고(60 고정)를 따르지 않는다** — 문턱까지 함께 내린 52 다.
            문턱 60 을 고정한 구성('유지 축소 단독')은 전체 구간 Calmar 0.90 으로 기각됐고,
            문턱 48~52 그리드가 두 구간 모두에서 기준선을 넘긴 plateau 였기 때문이다(규율 3).
            즉 권고는 '**분리 측정할 때** 크기와 자격을 섞지 말라'는 실험 규율이고, 채택값은
            그 분리 측정을 마친 뒤 그리드 검증으로 고른 52 다(`run_v2.FROZEN_RAMP`).
